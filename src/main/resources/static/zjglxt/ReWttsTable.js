...
$(document).on('click', ".queryButton",function(){
  $('#re_ts_table').bootstrapTable('refresh');
});

$("#re_ts_table").bootstrapTable({ // 对应table标签的id
	url: system_export_path+"/findByReState", // 获取表格数据的url
	...
	columns: [
		...
		},
		{
			...
			formatter: function (value, row, index) {
				return '<button class="btn btn-primary btn-sm" onclick="update(\'' + row.tsId + '\')">修改</button>	&nbsp<button class="btn btn-primary btn-sm" onclick="approve(\'' + row.tsId + '\')">通过</button>&nbsp<button class="btn btn-primary btn-sm" onclick="refuse(\'' + row.tsId + '\')">拒绝</button>' ;								  
			}
		}
	],
...
function approve(ids){
	$.ajax({
		type: 'get',
		url: system_export_path+"/approveWtts/"+ids,
		cache: false,
		processData: false,
		contentType: false,
		}).success(function (data) {
...
function dennyBatch() {
    //获取所有被选中的记录
    var rows = $("#re_ts_table").bootstrapTable('getSelections');
    if (rows.length== 0) {
        alert("请先选择要审核的记录!");
        return;
    }
...
function refuse(ids){
	$.ajax({
		type: 'get',
		url: system_export_path+"/refuseWtts/"+ids,
		cache: false,
		processData: false,
		contentType: false,
		}).success(function (data) {
