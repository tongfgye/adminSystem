...
$("#table").bootstrapTable({ // 对应table标签的id
	url: system_export_path+"/findByStateAndReStateWtxl", // 获取表格数据的url
	 ...
	 * 请求服务器数据时发送的参数，可以在这里添加额外的查询参数，返回false则终止请求 return { pageSize:
	 * params.limit, // 每页要显示的数据条数 offset: params.offset, // 每页显示数据的开始行号 sort:
	 * params.sort, // 要排序的字段 sortOrder: params.order, // 排序规则 dataId:
	 * $("#dataId").val() // 额外添加的参数 } },
	 */
	sortName: 'wid', // 要排序的字段
	sortOrder: 'asc', // 排序规则
	columns: [
		...
		},
		{
			...
			formatter: function (value, row, index) {
				if (row.reState == 1) {
				return '<button class="btn btn-primary btn-sm" onclick="update(\'' + row.wid + '\')">修改</button>	&nbsp<button class="btn btn-primary btn-sm" onclick="refuse(\'' + row.wid + '\')">删除</button>';								  
...
/*
 * $("#newSave").click(function(){ $("#myModal").modal(); $("#wid").val("");
 * $("#zbbh").val(""); $("#wtbh").val(""); $("#wtnr").val("");
 * $("#xlbh").val(""); $("#xlbz").val("");
 *  })
 */					
/*
